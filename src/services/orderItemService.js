...
export async function getOrderItemById(id) {
  const data = await apiClient.getById(RESOURCE_PATH, id)
...
export async function getOrderItemsByOrder(orderId) {
  ...
  const data = await apiClient.get(`${RESOURCE_PATH}?${params.toString()}`)
...
export async function createOrderItem(orderItem) {
  ...
  const data = await apiClient.post(RESOURCE_PATH, apiOrderItem, {
    headers: {
...
export async function updateOrderItem(id, updates) {
  ...
  const data = await apiClient.patch(`${RESOURCE_PATH}?id=eq.${id}`, apiOrderItem, {
    headers: {
      Prefer: 'return=representation',
    },
  })
