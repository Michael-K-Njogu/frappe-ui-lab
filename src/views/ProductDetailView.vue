...
async function deleteCurrentProduct() {
  ...
  try {
    ...
    info(`Product ${product.value.name} deleted successfully.`)
...
<template>
  <PageTitle title="Product Details" :has-back-button="true">
    <template #actions>
      ...
      <BaseButton v-if="product" label="Delete Product" variant="danger" :loading="deleting">
        <template #icon>
          <Trash2 size="16" />
        </template>
      </BaseButton>
  ...
  <ProductCard v-if="product" submit-label="Save Product" :product="product" />
</template>
